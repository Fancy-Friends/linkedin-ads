...
npm run provider -- linkedin-ads            # regenerate this repo
npm run provider -- linkedin-ads --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/linkedin-ads-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/linkedin-ads-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/linkedin-ads-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-linkedin-ads` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/linkedin-ads/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/linkedin-ads` |
| npm | `@particle-academy/linkedin-ads-ui`, `@particle-academy/linkedin-ads-js` |
| Packagist | `particle-academy/linkedin-ads-php` |
| PyPI | `fancy-linkedin-ads` |
...
**Default to plain HTTP.** A LinkedIn Ads SDK is third-party code and is subject to
