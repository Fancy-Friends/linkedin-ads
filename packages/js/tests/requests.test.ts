...
/**
 * What LinkedIn Ads actually receives.
 *
...
import { linkedinAdsCampaignGet } from "../src/actions/campaign-get.js";
...
test("campaign_get sends GET /rest/adAccounts/{adAccountId}/adCampaigns/{campaignId}", async () => {
  const { seen, transport } = capture();

  await linkedinAdsCampaignGet({
    config: {
      "adAccountId": "example-adAccountId",
  ...
  assert.ok(new URL(seen[0]!.url).pathname.endsWith("/rest/adAccounts/example-adAccountId/adCampaigns/example-campaignId"), seen[0]!.url);
